...
void ft_putvet(int vet[],
               int n) // função para imprimir os caracteres de um vetor
{
  ...
  {
    ft_putchar(vet[c] +
               '0'); // imprime o valor armazenado na posição c do vetor
...
void ft_print_combn(int n) {
  int combn[n];  // vetor usado para armazenar os algarismos das combinações a
                 // serem impressas
  int c;         // contador do while (loop)
  ...
  {
    c = 0;        // contador do while (loop) inicia em 0
    while (c < n) // loop para atribuir valores ao vetor da primeira combinação
                  // (012...)
    {
      combn[c] = c; // atribuição de valor de cada dígito para compor o vetor da
                    // primeira combinação
      c++;
    }
    while (ft_get_index(combn, n) !=
           -1) // loop para imprimir os vetores de todas combinações exceto a
               // última, seguidos de ", "
    {
    ...
    ft_putvet(
        combn,
        n); // imprime o vetor da última combinação da sequência (ex.: 789)
  }
}
